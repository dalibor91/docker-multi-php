...
then 
  ...
  then 
    ...
    echo "Enter container image (If empty 'apache2/php7')"
    read container_image
    if [ "$container_image" = "" ]; then container_image="apache2/php7"; fi

    docker build -t "$container_image" -f Dockerfile-7 .
...
then 
  ...
  then 
      ...
      echo "Enter container name (If empty 'apache2/php7')"
      read container_name
      if [ "$container_name" = "" ]; then container_name="apache2/php7"; fi
   
      echo "Enter hostname (If empty 'php7')"
      read host_name
      if [ "$host_name" = "" ]; then host_name="php7"; fi     
      ...
      share=""
      for dir in $(ls "${APP_DIR}/apps/7");
      do 
        echo "Share ${APP_DIR}/apps/7/$dir ? y/n"
        read yesorno
        if [ "$yesorno" = "y" ]; then share="${share} ${APP_DIR}/apps/7/${dir}:/var/www/apps/${dir}"; fi;
      done
